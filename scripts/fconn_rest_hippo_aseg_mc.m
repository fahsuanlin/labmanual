...
%100206_native_hippo_regressors_aseg_rest1_LR_hippo_left.mat
	'native_hippo_regressors_rest_01_hippo_left', 'native_hippo_regressors_rest_01_hippo_right';
        'native_hippo_regressors_rest_01_hippo_left', 'native_hippo_regressors_rest_01_hippo_right';
        'native_hippo_regressors_rest_01_hippo_left', 'native_hippo_regressors_rest_01_hippo_right';
        'native_hippo_regressors_rest_01_hippo_left', 'native_hippo_regressors_rest_01_hippo_right';
        'native_hippo_regressors_rest_01_hippo_left', 'native_hippo_regressors_rest_01_hippo_right';
...
for d_idx=1:length(subject)
	...
	ROI_std=[];
		...
		D_reg(:,1)=regressor_ventricle(1:size(STC,2));
		D_reg(:,2)=regressor_wm(1:size(STC,2));
                D_reg(1:n_dummy,:)=[];
	    ...
	    for r_idx=1:length(fstem_reg(f_idx,:))
			...
			%regressor_hippo=STC(9884,:)';
				...
				%tmp=regressor_hippo(1:size(STC,2));
                                %tmp=regressor_hippo;
                                %tmp=tmp(:)-D*inv(D'*D)*D'*tmp(:);

				tmp=regressor_hippo_full(:,1:size(STC,2));
...
save fconn_rest_hippo_aseg_mc.mat fconn
