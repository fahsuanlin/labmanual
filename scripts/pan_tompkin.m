function [qrs_amp_raw,qrs_i_raw,delay, ecg_h]=pan_tompkin(ecg,fs,gr,varargin)
...
for i=1:length(varargin)/2
    ...
    switch lower(option)
        ...
        case 'flag_auto_outlier'
            flag_auto_outlier=option_value;
        otherwise
...
if(flag_auto_outlier)
    %outlier detection
    y=quantile(ecg,11);
    idx=find(ecg<y(3).*5);
    ecg(idx)=randn(size(idx)).*eps;
    idx=find(ecg>y(9).*5);
    ecg(idx)=randn(size(idx)).*eps;
end;
...
% 200 msec distance
[pks,locs] = findpeaks(ecg_m,'MINPEAKDISTANCE',round(0.6*fs));
locs0=locs;
...
for i = 1 : length(pks)
    ...
    if pks(i) >= THR_SIG
        ...
        if length(qrs_c) >= 3
            if (locs(i)-qrs_i(end)) <= round(0.3600*fs)
                ...
                if abs(Slope1) <= abs(0.5*(Slope2))  % slope less then 0.5 of previous R
                    ...
                    NOISE_LEV = 0.01*pks(i) + 0.99*NOISE_LEV;
        ...
        if skip == 0  % skip is 1 when a T wave is detected
            ...
            %if(tt(locs(i)-round(0.150*fs)+x_i-1)>32.7) keyboard; end;
...
%                 hold on,plot(tt(locs(1:length(NOISL_buf1))),NOISL_buf1,'LineWidth',2,'Linestyle','--','color','k');
%                 hold on,plot(tt(locs(1:length(SIGL_buf1))),SIGL_buf1,'LineWidth',2,'Linestyle','-.','color','r');
%                 hold on,plot(tt(locs(1:length(THRS_buf1))),THRS_buf1,'LineWidth',2,'Linestyle','-.','color','g');
%                 hold on,plot(tt(locs(i)-round(0.150*fs)+x_i),y_i,'co','linewidth',2);
%                 keyboard;
            end
            yi_buf=[yi_buf, y_i];
            xi_buf=[xi_buf, tt(locs(i)-round(0.150*fs)+x_i)];
            
            %%fhlin graph
            %figure(1);
            %h1=subplot(211); plot(ecg); hold on; plot(qrs_i_raw,ecg(qrs_i_raw),'bo');
            %h2=subplot(212); plot(ecg_h); hold on; plot(qrs_i_raw,ecg_h(qrs_i_raw),'ro');
            %linkaxes([h1 h2],'x');
            
            % adjust Signal level
            %SIG_LEV = 0.125*pks(i) + 0.875*SIG_LEV ;
            SIG_LEV = 0.01*pks(i) + 0.99*SIG_LEV ;
        end
    ...
    elseif pks(i) < THR_NOISE
        ...
        %adjust Noise level in MVI
        %NOISE_LEV = 0.125*pks(i) + 0.875*NOISE_LEV;
        NOISE_LEV = 0.05*pks(i) + 0.95*NOISE_LEV;
...
if gr
    ...
    figure,az(1)=subplot(311);plot(tt,ecg_h);title('QRS on Filtered Signal');axis tight;
    hold on,scatter(tt(qrs_i_raw),qrs_amp_raw,'m');
    hold on,scatter(xi_buf,yi_buf,'c');
    hold on,plot(tt(locs(1:length(NOISL_buf1))),NOISL_buf1,'LineWidth',2,'Linestyle','--','color','k');
    hold on,plot(tt(locs(1:length(SIGL_buf1))),SIGL_buf1,'LineWidth',2,'Linestyle','-.','color','r');
    hold on,plot(tt(locs(1:length(THRS_buf1))),THRS_buf1,'LineWidth',2,'Linestyle','-.','color','g');
