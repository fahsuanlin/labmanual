...
select_channel={ 'Fp1'    'Fp2'    'F3'    'F4'    'C3'    'C4'    'P3'    'P4'    'O1'    'O2'    'F7'    'F8'    'T7'    'T8'    'P7'    'P8'    'Fz'    'Cz'    'Pz'    'IO'    'FC1'    'FC2'    'CP1'    'CP2'    'FC5'    'FC6'    'CP5'    'CP6'    'FT9'    'FT10'    'TP9'    'TP10'};
...
flag_reref= [1 1 1]; %referencing: remove the average time course across all electrodes from the time course at each individual electrode
flag_hp=    [1 1 1]; %high pass filtering at 0.1 Hz
flag_aas=   [0 0 0]; %AAS for gradient artifact suppression
flag_bcg=    [0 0 0]; %BCG artifact suppression

time_trim=[30];     %trim off the first few seconds...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% ERP setup
%
...
for f_idx=1:length(headerFile)
    ...
    channel_select=ones(1,size(eeg{f_idx},1)).*nan;
    ...
    ecg_channel_idx=channel_select(ecg_channel);
    
    eeg_channel_idx=channel_select(eeg_channel);
        
    tmp=zeros(length(ecg_channel_idx),size(eeg{f_idx},2));
    for ch_idx=1:length(ecg_channel_idx)
        if(~isnan(ecg_channel_idx(ch_idx)))
            tmp(ch_idx,:)=eeg{f_idx}(eeg_channel_idx(ch_idx),:);
        end;
    end;
    ecg{f_idx}=tmp;
    
    
    tmp=zeros(length(eeg_channel_idx),size(eeg{f_idx},2));
    for ch_idx=1:length(eeg_channel_idx)
        if(~isnan(eeg_channel_idx(ch_idx)))
            tmp(ch_idx,:)=eeg{f_idx}(eeg_channel_idx(ch_idx),:);
        end;
    end;
    eeg{f_idx}=tmp;
    ...
    if(~isempty(ecg{f_idx}))
        ...
        if(flag_bcg(f_idx))
            ...
            [eeg{f_idx}, qrs_i_raw, bcg_all, ecg_all, bad_trials]=eeg_bcg(eeg{f_idx},ecg{f_idx},fs(f_idx),'flag_display',0,'bcg_nsvd',3,'n_ma_bcg',20,'flag_anchor_ends',0,'flag_post_ssp',0); %BCG correction on EEG
        end;
    ...
    if(~isempty(trigger{f_idx}))
        ...
        for event_idx=1:length(erp_event)
            ...
            if(f_idx==1)
                erp_c_all(f_idx,event_idx).data=[];
            end;
            erp_c_all(f_idx,event_idx).data=cat(3,erp_c_all(f_idx,event_idx).data,erp{f_idx,event_idx}.trials(:,1:time_pre,:));
...
for event_idx=1:length(erp_event)
    for f_idx=1:length(headerFile)
        tmp=erp_c_all(f_idx,event_idx).data;
        if(f_idx==1)
            tmp=reshape(tmp,[size(tmp,1),size(tmp,2)*size(tmp,3)]);
        else
            tmp=cat(2,tmp,reshape(tmp,[size(tmp,1),size(tmp,2)*size(tmp,3)]));
        end;
    end;
    C(event_idx).C=cov(tmp');
end;
            

%save(output_file,'erp','fs','found_channel');

save(output_avg_file,'erp','fs','found_channel','C');
