...
root_path='/space_lin2/fhlin/eegmri_memory';
...
setenv('SUBJECTS_DIR','/space_lin2/fhlin/eegmri_memory/subjects');
                ...
                try
                    ...
                    for region_idx=1:length(region_index)
                        ...
                        for aseg_idx=1:length(file_hippo_aseg)
                            ...
                            eval(sprintf('!mri_vol2vol --nearest --mov %s --targ %s/%s/mri/%s --reg %s --o  %s/%s/resting_analysis/%s-aseg.mgz --inv ',file_mov, subjects_dir,subject{f_idx}, file_hippo_aseg{v_idx}, file_register,root_path,subject{f_idx},fmri_vol_data{v_idx}));
