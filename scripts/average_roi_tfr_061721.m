...
output_stem='average_roi_tfr_061721';
...
for roi_idx=1:size(tfr,1)
    for cond_idx=1:size(tfr,2)
        tfr_all_lh_Z(:,:,roi_idx,cond_idx)=etc_z(squeeze(mean(tfr_all(:,:,lh_subj,roi_idx,cond_idx),3)),base_idx,'flag_baseline_correct',1,'dim',1);
        
        
        figure;
        h=pcolor(timeVec,log10(freqVec),tfr_all_lh_Z(:,:,roi_idx,cond_idx)'); shading flat; set(gca,'clim',clim);
