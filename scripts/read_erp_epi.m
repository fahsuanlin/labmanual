...
flag_reref= [1 1 1 1 1 1]; %referencing: remove the average time course across all electrodes from the time course at each individual electrode
...
%output_file='erp.mat';
output_avg_file='erp_avg_epi.mat';
...
for f_idx=1:length(headerFile)
    ...
    if(~isempty(time_trim))
        ...
        ch20_after_aas=ch20_after_aas(:,time_trim_idx+1:end);
