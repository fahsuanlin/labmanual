...
output_stem='tfMRI_WM_surf_soa_can3_glm';
%------------------------------------------------------------------------------------
...
for subj_idx=1:length(subject)
    ...
    if(~flag_file_error)
        ...
        if(flag_hdr_canonical)
            ...
            HDR=fmri_hdr(hhdr_timeVec,'hdr_order',3);
            HDR(find(hhdr_timeVec<0),:)=0;
            %tmp=sum(HDR.^2,1);
            %HDR=HDR./repmat(sqrt(tmp),[size(HDR,1),2]);

            hhdr_timeVec0=[0:TR:hdr_length-TR];
            HDR0=fmri_hdr(hhdr_timeVec0,'hdr_order',3);
            HDR0(find(hhdr_timeVec0<0),:)=0;
            %tmp=sum(HDR0.^2,1);
            %HDR0=HDR0./repmat(sqrt(tmp),[size(HDR0,1),2]);
        elseif(flag_hdr_fir)
...
%             %etc_save_nii(beta,nii,sprintf('%s_beta.nii',output_stem));
            ...
            fn=sprintf('%s/%s/analysis/%s_h%02d_beta-rh.stc',root_path,subject{subj_idx},output_stem,h);
            inverse_write_stc(w_beta(1:size(contrast_hdr,2),length(v_lh)+1:end)',v_rh,0,TR.*1e3,fn);
