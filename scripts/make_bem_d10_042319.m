...
%sesnor info
file_meg_mat='./meg_fwd_prep_042319.mat';
...
% fprintf(fp,'Meshes 3\n');
% fprintf(fp,'Mesh: "./cortex.tri"\n');
% fprintf(fp,'Mesh: "./skull.tri"\n');
% fprintf(fp,'Mesh: "./scalp.tri"\n');
% fprintf(fp,'Interfaces 3\n');
% fprintf(fp,'Interface cortex: 1\n');
% fprintf(fp,'Interface skull: 2\n');
% fprintf(fp,'Interface scalp: 3\n');
fprintf(fp,'Interfaces 3\n');
...
%make SQUID sensor file
fprintf('making SQUID sensor file...\n');
load(file_meg_mat); %including variable 'n_meg'

fp=fopen(sprintf('%s.squids',output_stem),'w');
for v_idx=1:n_meg
    
    nn=points(v_idx+n_meg,:)-points(v_idx,:);

    fprintf(fp,'AG%03d %f %f %f %f %f %f -1\n',v_idx,points(v_idx,1).*1e3, points(v_idx,2).*1e3,points(v_idx,3).*1e3,nn(1), nn(2), nn(3));
    fprintf(fp,'AG%03d %f %f %f %f %f %f 1\n',v_idx,points(n_meg+v_idx,1).*1e3, points(n_meg+v_idx,2).*1e3,points(n_meg+v_idx,3).*1e3,-nn(1), -nn(2), -nn(3));
