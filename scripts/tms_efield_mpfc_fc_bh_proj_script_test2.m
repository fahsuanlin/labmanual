...
root_dir='/Users/fhlin/workspace/eegmri_memory/s016/tms_e_field';
%root_dir='/space_lin2/fhlin/eegmri_memory/s016/tms_e_field';
...
output_stem='mri_b91_mpfc_fc_bh_proj_efield_test2';

target_coord=[-13 56 35];
targeg_coord_mni=[-3 58 36]; %fc
...
for subj_idx=1:length(subject)
    ...
    %specify 'strcoil' in this function; if not, it will load from the base
    %workspace
...
rotation_deg=[15:15:360-1];


tms_coil_xfm_moved_tuned_before=tms_coil_xfm_moved;
for rot_idx=1:length(rotation_deg)
    [tms_coil_xfm_moved_tuned_after]=etc_tms_target_xfm_tune(target_coord, bem_obj(head_surf_idx), tms_coil_center_now, tms_coil_axis_now, tms_coil_up_now, tms_coil_xfm_moved_tuned_before, 4, rotation_deg(rot_idx));

    %move strcoil
    [strcoil] = etc_tms_target_xfm_apply(strcoil, tms_coil_xfm_moved(1:3,4).*1e3, -tms_coil_xfm_moved(1:3,3), tms_coil_xfm_moved(1:3,2), tms_coil_xfm_moved_tuned_before, tms_coil_xfm_moved_tuned_after);

    %update tms coil xfm
    tms_coil_xfm_moved_tuned_before=tms_coil_xfm_moved_tuned_after;
    ...
    %append TMS coil object
    efield_rot{rot_idx}.tms_coil_xfm=tms_coil_xfm_moved_tuned_after;
