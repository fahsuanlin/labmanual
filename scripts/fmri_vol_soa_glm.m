...
file_nii={
    '../fmri_data/unpack/bold/030/sfmcprstc.nii';
    '../fmri_data/unpack/bold/032/sfmcprstc.nii';
    };
...
for f_idx=1:length(file_nii)
    nii=load_untouch_nii(file_nii{f_idx});
    nii_sz=size(nii.img);
    
    stc=cat(2,stc,reshape(nii.img,[size(nii.img,1)*size(nii.img,2)*size(nii.img,3) size(nii.img,4)]));
    if(~isempty(exclude_time))
        exclude_time_all=cat(1,exclude_time_all,exclude_time(:)+(size(stc0,2)).*(f_idx-1));
    end;
    
    timepoints(f_idx)=size(nii.img,4);
...
etc_save_nii(reshape(mean(stc,1),[nii_sz(1) nii_sz(2) nii_sz(3)]),nii,sprintf('%s_under.nii',output_stem));
...
for h=1:length(hypothesis)
    ...
    etc_save_nii(t_stat,nii,sprintf('%s_h%02d_tstat.nii',output_stem,h));

    tmp=reshape(w_beta',[nii_sz(1) nii_sz(2) nii_sz(3) size(w_beta,1)]);
    etc_save_nii(tmp,nii,sprintf('%s_h%02d_beta.nii',output_stem,h));
    ...
    if(flag_hdr_fir)
        ...
        mne=reshape(mne,[nii_sz(1) nii_sz(2) nii_sz(3)]);
        etc_save_nii(mne,nii,sprintf('%s_h%02d_mne.nii',output_stem,h));

        dspm=reshape(dspm,[nii_sz(1) nii_sz(2) nii_sz(3) size(beta,1)]);
        etc_save_nii(dspm,nii,sprintf('%s_h%0d2_dspm.nii',output_stem,h));        
    end;
end;
...
under=load_untouch_nii('fmri_vol_soa_glm_under.nii');
under=under.img;

over=load_untouch_nii('fmri_vol_soa_glm_h01_tstat.nii');
over=over.img;

fmri_overlay(under,over,'>',[3 6]);
return;
