...
root_path='/space_lin2/fhlin/sri_7t';
%root_path='/Users/fhlin/workspace/eegmri_fus';
...
setenv('SUBJECTS_DIR','/space_lin2/fhlin/sri_7t/subjects');
%setenv('SUBJECTS_DIR','/Users/fhlin/workspace/eegmri_fus/subjects');
...
fmri_vol_data={
	'../fmri_data/unpack/bold/037/fmcprstc_topup.nii.gz';
    };

output_fstem={
    'native_hippo_regressors_rest_01';
    }
            ...
            if(exist(file_mov))
                ...
                try
                    ...
                    for region_idx=1:length(region_index)
                        ...
                        file_output=sprintf('%s_%s.mat',output_fstem{v_idx},region_stem{region_idx});
                        ...
                        for aseg_idx=1:length(file_hippo_aseg)
                            ...
                            eval(sprintf('!mri_vol2vol --nearest --mov %s --targ %s/%s/mri/%s --reg %s --o  %s-aseg.mgz --inv ',file_mov, subjects_dir,subject{f_idx}, file_hippo_aseg{v_idx}, file_register{v_idx},fmri_vol_data{v_idx}));


                            %d_aseg=MRIread(sprintf('%s/%s/resting_analysis/%s-aseg.mgz',root_path, subject{f_idx},fmri_vol_data{v_idx}));
                            d_aseg=MRIread(sprintf('%s-aseg.mgz',fmri_vol_data{v_idx}));
