...
output_stem='isc_permswb_121419_z';
...
for sequence_idx=1:length(sequence)
    ...
    for hemi_idx=1:length(hemi)
        ...
        if(~isempty(find(isnan(cc(:))))) keyboard; end;
        
        inverse_write_stc(cc,a,b,c,sprintf('%s_gmr-%s.stc',oo,hemi{hemi_idx}));
        ...
        for perm_idx=1:n_perm
            for v_idx=1:size(stc,1)
                ...
                tmp_perm=tril(corrcoef(data).*perm_swb_mat,-1);
                ...
                ccm_perm(v_idx)=zm_perm;
                ccmd_perm(v_idx)=zmd_perm;                
            end;
            
            idx=find(ccm_perm>ccm);
            zm_perm_p(idx)=zm_perm_p(idx)+1;
            idx=find(ccmd_perm>ccmd);
            zmd_perm_p(idx)=zmd_perm_p(idx)+1;
            
        end;
        zm_perm_p=zm_perm_p./n_perm;
        zmd_perm_p=zmd_perm_p./n_perm;
        fprintf('\n');
